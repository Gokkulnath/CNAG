...
def fit(nb_epochs,target_model,dls,optimizer,adversarygen=adversarygen):
    ...
    for epoch in tqdm(range(nb_epochs),total=nb_epochs):
        ...
        for batch_idx, data in tqdm(enumerate(train_dl),total = train_num//train_dl.batch_size):
            ...
            # XB = images
            #preds_XB = f(images)
            prob_vec_clean = F.softmax(target_model(images),dim=0) # Variable q
            clean_preds ,clean_idx = get_preds(prob_vec_clean,return_idx=True,k=1)
            
            #XA = images+noise
            #preds_XA = f(images + noise)
            prob_vec_no_shuffle = target_model(images + noise)  
            ...
            prob_vec_shuffled =   target_model(images + noise[torch.randperm(bs)])
...
#                 wandb.log({"noise": [wandb.Image(im, caption=f"Noise_{arch}_{epoch}_{perturb_idx}")]})
            ...
            if batch_idx!=0  and batch_idx % 100 ==0 :
                ...
                fool_rate,total_fool= validate_generator(noise,target_model,val_dl)
